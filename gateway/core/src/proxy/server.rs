...
pub async fn handle_request(
    ...
    match (method.as_str(), path.as_str()) {
        ...
        ("POST", p) if p.starts_with("/v1/spire/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("GET", p) if p.starts_with("/v1/spire/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("PUT", p) if p.starts_with("/v1/spire/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("DELETE", p) if p.starts_with("/v1/spire/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("POST", p) if p.starts_with("/v1/skills") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("GET", p) if p.starts_with("/v1/skills") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("PUT", p) if p.starts_with("/v1/skills/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("DELETE", p) if p.starts_with("/v1/skills/") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("POST", p) if p.starts_with("/v1/agents/") && p.contains("/skills") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
        ("GET", p) if p.starts_with("/v1/agents/") && p.contains("/skills") => {
            crate::proxy::forward::forward_to_control_plane(req, state).await
        }
